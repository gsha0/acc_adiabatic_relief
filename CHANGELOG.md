...
### Added
- `COP_MAX` parameter in `config.py` (Operating Limits section) — hard upper limit on chiller COP, default 30. Applied as an EIR floor (`EIR ≥ 1/COP_MAX`) at the last step of the power calculation, before COP and plant power are derived. Prevents physically implausible results from performance curve extrapolation under very favourable conditions.
